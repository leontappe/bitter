    ...
    Row(
      ...
      children: <Widget>[
        Column(
          ...
          children: <Widget>[
            Paragraph(
                ...
                margin: EdgeInsets.only(bottom: 8.0)),
            if (vendor.smallBusiness && (vendor.contact?.isNotEmpty ?? false))
              Text(vendor.contact!,
                  style:
                      TextStyle(fontSize: fontSize, font: font, color: color))
            else if (vendor.manager == null ||
                (vendor.manager?.isEmpty ?? false))
              Text(vendor.name,
                  style:
                      TextStyle(fontSize: fontSize, font: font, color: color))
            else
        ...
        Column(
          ...
          children: <Widget>[
            Paragraph(
                text: vendor.taxNr?.isNotEmpty ?? false
                    ? 'Steuer-Nr.: ${vendor.taxNr}'
                    : '',
                style: TextStyle(fontSize: fontSize, font: font, color: color),
                margin: EdgeInsets.only(top: 17.0)),
            Text(
                vendor.vatNr?.isNotEmpty ?? false
                    ? 'USt.-ID: ${vendor.vatNr}'
                    : '',
                style: TextStyle(fontSize: fontSize, font: font, color: color)),
            Text(vendor.website ?? '',
                style: TextStyle(fontSize: fontSize, font: font, color: color)),
            if (vendor.freeInformation?.isNotEmpty ?? false)
