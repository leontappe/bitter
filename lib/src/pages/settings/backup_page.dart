...
class Operation {
  ...
  @override
  String toString() =>
      '[Operation - started:$started. finished:$finished, success:$success]';
}
...
class _BackupPageState extends State<BackupPage> {
  late BillRepository<DatabaseProvider> billRepo;
  late CustomerRepository<DatabaseProvider> customerRepo;
  late DraftRepository<DatabaseProvider> draftRepo;
  late ItemRepository<DatabaseProvider> itemRepo;
  late VendorRepository<DatabaseProvider> vendorRepo;
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    if (archivePath != null)
                      Text(' Aktuelle Auswahl: $archivePath'),
                    Divider(height: 16.0),
                    CheckboxListTile(
                      title: Text(
                          'Vorhandene Daten vor dem Wiederherstellen löschen?'),
                      value: overwriteRestore,
                      onChanged: (bool? input) {
                        if (input == null) return;
                        setState(() => overwriteRestore = input);
                      },
                    ),
                    Divider(height: 16.0),
                    ...
                    CheckboxListTile(
                      ...
                      onChanged: (bool? input) {
                        if (input == null) return;
                        setState(() => recoveryChoice.all = input);
                      },
                      title: Text('Alle'),
                    ),
                    CheckboxListTile(
                      ...
                      onChanged: (bool? input) {
                        if (input == null) return;
                        setState(() => recoveryChoice.bills = input);
                      },
                    ...
                    CheckboxListTile(
                      ...
                      onChanged: (bool? input) {
                        if (input == null) return;
                        setState(() => recoveryChoice.customers = input);
                      },
                      title: Text('Kunden'),
                    ),
                    CheckboxListTile(
                      ...
                      onChanged: (bool? input) {
                        if (input == null) return;
                        setState(() => recoveryChoice.drafts = input);
                      },
                      title: Text('Entwürfe'),
                    ),
                    CheckboxListTile(
                      ...
                      onChanged: (bool? input) {
                        if (input == null) return;
                        setState(() => recoveryChoice.items = input);
                      },
                      title: Text('Artikel'),
                    ),
                    CheckboxListTile(
                      ...
                      onChanged: (bool? input) {
                        if (input == null) return;
                        setState(() => recoveryChoice.vendors = input);
                      },
                    ...
                    Row(
                      children: [
                        ElevatedButton(
                            ...
                            child: Text('Wiederherstellung starten')),
                        Spacer(),
                        if (busy) CircularProgressIndicator(),
                      ],
                    ),
  ...
  void onStartBackup() async {
    ...
    if (bills.isNotEmpty) {
      ...
      await File('${csvDir.path}/bills.csv')
          .writeAsString(csvConverter.convert(billCsv));
    }

    if (customers.isNotEmpty) {
      final customerCsv = <List<dynamic>>[customers.first.toMap.keys.toList()];
      customerCsv
          .addAll(customers.map<List<dynamic>>((e) => e.toMap.values.toList()));
      await File('${csvDir.path}/customers.csv')
          .writeAsString(csvConverter.convert(customerCsv));
    }

    if (drafts.isNotEmpty) {
      final draftCsv = <List<dynamic>>[drafts.first.toMap.keys.toList()];
      draftCsv
          .addAll(drafts.map<List<dynamic>>((e) => e.toMap.values.toList()));
      await File('${csvDir.path}/drafts.csv')
          .writeAsString(csvConverter.convert(draftCsv));
    }

    if (items.isNotEmpty) {
      final itemCsv = <List<dynamic>>[items.first.toMap.keys.toList()];
      itemCsv.addAll(items.map<List<dynamic>>((e) => e.toMap.values.toList()));
      await File('${csvDir.path}/items.csv')
          .writeAsString(csvConverter.convert(itemCsv));
    }

    if (vendors.isNotEmpty) {
      final vendorCsv = <List<dynamic>>[vendors.first.toMap.keys.toList()];
      vendorCsv
          .addAll(vendors.map<List<dynamic>>((e) => e.toMap.values.toList()));
      await File('${csvDir.path}/vendors.csv')
          .writeAsString(csvConverter.convert(vendorCsv));
    }
  ...
  void onStartRecovery() async {
    ...
    final shouldStartRecovery = await showDialog<bool>(
        context: context,
        builder: (BuildContext context) => AlertDialog(
              title:
                  Text('Möchtest du wirklich die Wiederherstellung starten?'),
              actions: [
                ElevatedButton(
    ...
    if (!(shouldStartRecovery ?? false)) return;
    ...
    setState(
        () => restores.first.finish(result: 'Wiederherstellung abgeschlossen'));
  }
  ...
  List<Map<String, dynamic>> _mapFileContent(Uint8List file) {
    ...
    for (var i = 1; i < csvList.length; i++) {
      ...
      map.removeWhere(
          (String key, dynamic value) => (value == null || value == 'null'));
      mappedList.add(map);
    }
    return mappedList;
  }
